...
public class ActionManager {
	...
	/*
	 * Process an update request that came either from an anonymous form or from the managed forms page
	 */
		...
		try {
			...
			for(int i = 0; i < updates.size(); i++) {
				...
				if(tc.actions != null && tc.actions.size() > 0) {
					...
					applyManagedFormActions(request, sd, tc, oId, sId, pId, managedId, u.prikey, priority, u.value, localisation);
				}
